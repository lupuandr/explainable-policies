...
def schedule_job_brax(env, seed, folder, args):
...
def schedule_job_minatar(env, seed, folder, args):
    if not os.path.exists(folder):
        os.makedirs(folder)
...
with executor.batch():
    for env in ["SpaceInvaders-MinAtar", "Breakout-MinAtar", "Asterix-MinAtar", "Freeway-MinAtar"]:
        for seed in [0, 1, 2]:
            folder = f"{root}/{env}/seed{seed}/"
            job = schedule_job_minatar(env, seed, folder)
            jobs.append(job)
